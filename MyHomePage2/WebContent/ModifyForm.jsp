...
<link rel="stylesheet" type="text/css" href= "${contextPath}/css/modify.css?ver=3">
...
function setConfig(text){
	config = { txHost: '', /* 런타임 시 리소스들을 로딩할 때 필요한 부분으로, 경로가 변경되면 이 부분 수정이 필요. ex) http://xxx.xxx.com */ 
					txPath: '', /* 런타임 시 리소스들을 로딩할 때 필요한 부분으로, 경로가 변경되면 이 부분 수정이 필요. ex) /xxx/xxx/ */ 
					txService: 'sample', /* 수정필요없음. */ 
					txProject: 'sample', /* 수정필요없음. 프로젝트가 여러개일 경우만 수정한다. */ 
					initializedId: "", /* 대부분의 경우에 빈문자열 */ 
					wrapper: "tx_trex_container", /* 에디터를 둘러싸고 있는 레이어 이름(에디터 컨테이너) */ 
					form: 'modifyBoard'+"", /* 등록하기 위한 Form 이름 */ 
					txIconPath: path+"/daumeditor/images/icon/editor/", /*에디터에 사용되는 이미지 디렉터리, 필요에 따라 수정한다. */ 
					txDecoPath: path+"/daumeditor/images/deco/contents/", /*본문에 사용되는 이미지 디렉터리, 서비스에서 사용할 때는 완성된 컨텐츠로 배포되기 위해 절대경로로 수정한다. */ 
					canvas: { styles: 
								{ color: "#123456", /* 기본 글자색 */ 
...
<body>
	<div id = "wrap" >
		<form action = "board" name = "modifyBoard" id = "myFrm" >
			<input type = "hidden" name = "cmd" value = "writeModify">
			<input type = "hidden" name = "num" value = "${board.b_id}">
			<input type = "hidden" id = "cont" name = "context" value = "${board.context}">
			<table>
				<tr>
...
<!-- 						<textarea name = "context" id = "context"></textarea> -->
				...
				<tr>
					<td colspan = "2" class = "ops" style = "text-align : right">
						...
						<button onclick = "Editor.save();">작성</button>
					</td>
				</tr>
			</table>
		</form>
