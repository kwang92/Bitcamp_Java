...
public class MyManagement2{
	...
	private ReadStuInfo read;
	private WriteStuInfo write;
	
	public MyManagement2() {		// MyManagement 클래스 생성자 
		input = new Scanner(System.in);
		stu = new MyList<Student>();
		read = new ReadStuInfo();
		write = new WriteStuInfo();
		this.readValue();
	}
	...
	public void start() {	// 프로그램 start 메소드 ( 최초 Menu 선택 )
		...
		do {
			...
			case 5:
				...
				write.writeStuInfo_ToFile(this.stu);
				boot = false;
				break;
			default:
	...
	public void stuAdd() {	// 학생 추가 메소드, 배열크기가 꽉차면 원래크기 *2 크기 만큼의 배열로 바꿔준다.
		...
		this.stu.add(s);	// Student_List의 add 메소드 호출
	}
